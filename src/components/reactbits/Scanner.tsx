// Vendored from React Bits (https://reactbits.dev/backgrounds/scanner), MIT licensed.
// Left close to the upstream source (see eslint.config.js globalIgnores) so future updates can
// be dropped in with minimal diffing — only the `.jsx` -> `.tsx` typing was added.
...
const directionToFloat = (dir: ScanDirection) => (dir === 'horizontal' ? 1.0 : dir === 'diagonal' ? 2.0 : 0.0);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try {
        ...
        // canvas already detached (e.g. StrictMode double-invoke) — nothing to clean up
      }
